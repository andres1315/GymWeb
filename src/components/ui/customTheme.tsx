...
const HoverBorderGradient = ({
  ...
  return (
    <div
      className={cn(
        "group relative rounded-md p-[1px] overflow-hidden",
        containerClassName
      )}
      {...props}
    >
      ...
      <div className={cn("relative rounded-[calc(0.5rem-1px)]", className)}>
        {children}
      </div>
    </div>
  );
...
function ButtonSecondary({
  ...
  className,
  ...props
}: React.ComponentProps<"button"> & PropsButtonSecondary) {
  return (
    <Button
      type="button"
      className={cn(
        "bg-white/10 border-white/20 text-white hover:bg-white/20 py-5",
        className
      )}
      {...props}
    >
