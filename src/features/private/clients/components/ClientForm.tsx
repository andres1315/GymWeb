...
import { AssignMembership } from "./form/assign-membersip";
import { useMembershipQuery } from "../../membership/hooks/useMembershipQuery";
import type { MembershipSaved } from "../../membership/models/MembershipSaved";
...
export const clientSchema = z.object({
  ...
  blood_type_id: z.number().int().optional().nullable(),
  ...
  memberships: z.array(z.object({
    id: z.number().int(),
    name: z.string(),
    price_plan: z.number().int(),
    membership_info: z.object({
  ...
  membership_to_assign: z.string().optional(),
});
...
console.log(currentClient);
  useEffect(() => {
    const optionsListMembership =
      ListMembership?.data
        ?.filter((plan: MembershipSaved) => plan.is_active)
        ?.map((plan: MembershipSaved) => ({
          value: plan.id,
          label:
            plan.name +
            " - " +
            ...
            }).format(plan.price_plan),
        })) || [];
    setMemberships(optionsListMembership);
  }, [ListMembership]);
  ...
  useEffect(() => {
    if (currentClient) {
      form.reset({
        ...
        membership_to_assign: currentClient.membership_to_assign ?? "",
        memberships: currentClient.memberships ?? [],
      });
    } else {
      form.reset({
        enrollment_date: format(new Date(), "yyyy-MM-dd"),
          ...
          <div className="flex justify-end gap-3">
            {isSaving ? (
              ...
              <>
                <Button
                  variant="outline"
                  onClick={() => setActionModule("dashboard")}
                  type="button"
                >
                  Cancelar
                </Button>
                <Button type="submit">
                  {actionModule === "create" ? "Guardar" : "Actualizar"}
                </Button>
              </>
            )}
          </div>
